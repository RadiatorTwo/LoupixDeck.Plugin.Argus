...
/// <summary>
/// Samples the Argus snapshot once a second into a <see cref="TelemetryFrame"/>: every sensor and
...
internal sealed class TelemetrySampler(ArgusMonitorService argus, Func<double> tjMax) : IDisposable
{
    ...
    private TelemetryFrame BuildFrame()
    {
        ...
        {
            int ordinal = ordinals.GetValueOrDefault(sensor.Type);
            ordinals[sensor.Type] = ordinal + 1;
            inputs.Add((MetricKeys.ForSensor(sensor.Type, ordinal), SensorMetrics.NativeValue(sensor),
